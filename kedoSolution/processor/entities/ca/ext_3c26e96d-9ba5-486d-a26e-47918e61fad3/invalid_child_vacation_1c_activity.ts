...
interface IInvalidChildVacation {
	staff: IStaffData,
	start_date: string,
	end_date: string,
	parent_info: string,
	care_days: { day: string}[],
	comment?: string,
}
...
async function action(): Promise<void> {
	...
	const vacation_data: IInvalidChildVacation = JSON.parse(Context.data.vacation_data);
	...
	const body = {
		...
		"СведенияОВторомРодителе": vacation_data.parent_info,
		"ПорядокВыплаты": "Аванс",
		"ДниУхода": vacation_data.care_days.map(f => {
			return {
				"Дата" : f.day
			}
		}),
	...
	integration_app.data.__name = `Оплата дней ухода за детьми-инвалидами (${vacation_data.staff.name})`;
	integration_app.data.document_odata_name = "Document_ОплатаДнейУходаЗаДетьмиИнвалидами";
