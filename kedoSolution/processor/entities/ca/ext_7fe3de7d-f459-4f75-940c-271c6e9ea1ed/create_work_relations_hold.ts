...
async function action(): Promise<void> {
	...
	const body = {
		...
		"Организация_Key": Context.data.org_id,
		"ДатаУвольнения": `${Context.data.dismissal_date!.format('YYYY-MM-DD')}T00:00:00`,
		"Сотрудник_Key": Context.data.guid,
		"ФизическоеЛицо_Key": Context.data.personal_id,
		"ОснованиеУвольнения": Context.data.dismissal_base,
	...
	if (!Context.data.alternative_way) {
		...
		const resUrl = `${baseUrl}/Document_ПриостановлениеТрудовыхДоговоровКонтрактов?$format=json`;
		...
		awaitingApp.data.__name = "Увольнение"
		awaitingApp.data.document_odata_name = "Document_Увольнение"
		...
		awaitingApp.data.base_1c_name = Context.data.base_1c_name;
