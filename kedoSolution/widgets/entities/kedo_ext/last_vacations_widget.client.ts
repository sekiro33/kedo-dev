...
class DomManager {
    ...
    staff: ApplicationItem<Application$kedo$staff$Data, any>;
...
async function init(): Promise<void> {
    ...
    domManager.staff = staff;

    // const vacationsRestLink = document.querySelector(".vacations-page_main-content_title-button");
    // vacationsRestLink.href = `${window.location.href}(p:item/kedo/staff/${staff.id})`;
    domManager.parser = new DOMParser();

    if (!window.closestVacationsRefreshInterval && Context.data.refresh_data) {
        window.closestVacationsRefreshInterval = window.setInterval(() => {
            getData(true);
            console.log("closest vacations refresh");
        }, 60000);
    };
};

async function onLoad(): Promise<void> {
    domManager.elementLoader = document.querySelector("[data-loader=vacation_widget]");
    getData();
}

async function getData(refresh = false): Promise<void> {
    domManager.allStatuses = await Context.fields.kedo_statuses.app.search().where(f => f.__deletedAt.eq(null)).size(100).all();
    domManager.allVacations = await Context.fields.closest_vacations.app.search().where((f, g) => g.and(
        f.__deletedAt.eq(null),
        f.kedo_staff.link(domManager.staff)
    )).size(3).sort("start").all();

    renderPage(refresh);
};

async function renderPage(refresh = false): Promise<void> {
    if (refresh) {
        domManager.vacationContainer.innerHTML = "";
    };
    domManager.vacationNodeTemplate = document.querySelector(".personnel-events-widget_item-template");
    domManager.vacationContainer = document.querySelector(".vacations-page_main-content-widget_container");

    domManager.allVacations.forEach(vacation => {
        const [startDay, startMonth, startYear] = vacation.data.start_string ? vacation.data.start_string.split(".") : ["", "", ""];
        const [endDay, endMonth, endYear] = vacation.data.end_string ? vacation.data.end_string.split(".") : ["", "", ""];
        const vacationStatus = domManager.allStatuses.find(status => status.id === vacation.data.kedo_status?.id);
        ...
        try {
            elementData = {
                ...
                iconContent: svgToTypeReference.vacation,
                extraContentClass: "item-shortand",
                extraContent: issueCodeToNameReference["holidays"],
                status: vacationStatus ? vacationStatus.data.__name : "Не определён",
                link: `${domManager.baseUrl}(p:item/absences/vacations/${vacation.id})`
            };
        } catch (e) {
            elementData = {
                titleClass: "item-header",
        ...
        const newIssueItem = domManager.vacationNodeTemplate.content.cloneNode(true).querySelector(".personnel-events-widget_item");
        domManager.renderComponent(domManager.vacationContainer, newIssueItem, false, elementData);
    });
};