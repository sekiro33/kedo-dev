...
let issues_manager: IssuesManager;
...
class IssuesManager {
    ...
    async renderWidget(refresh = false) {
        ...
        // Получение контейнера и шаблона заявок
        const issuesContainer = document.querySelector('.main-page_issues-widget').querySelector(".main-page_issues-widget_container");
        const issueItemTemplate = document.querySelector(".issues-widget_item-template");

        refresh && refreshContainers([issuesContainer]);

        // Получение контейнера с текстом про отсутствия заявок
        const emptyIssuesTemplate = document.querySelector(".main-page_issues-widget_container_empty");

        // Если заявок нет, то отображаем контейнер с текстом, если есть, то прячем
        if (!userDataManager.issues || userDataManager.issues.length < 1 || userDataManager.lastDocsForRender.length === 0) {
        ...
        userDataManager.lastDocsForRender.forEach(item => {
            ...
            const newIssueContent = issueItemTemplate.content.cloneNode(true);
            const newIssueElement = newIssueContent.querySelector(".issues-widget_item");
            this.renderComponent(issuesContainer, newIssueElement, false, elementData);
        });
        ...
        const all_new_bloks_hidden = issuesContainer.querySelectorAll('.issues-widget_item.hidden');
...
async function getOrRefreshData(refresh = false): Promise<void> {
    ...
    // Загрузка всех заявок сотрудника
    const allIssues = await Promise.all([
        ...
        Context.fields.medical_request_app.app.search().where(defaultFilter).size(1000).all(),
        Context.fields.transfer_application_app.app.search().where(defaultFilter).size(1000).all(),
        Context.fields.execution_duties_app.app.search().where(defaultFilter).size(1000).all(),
        Context.fields.dismissal_app.app.search().where(defaultFilter).size(1000).all(),
        Context.fields.employment_app.app.search().where(defaultFilter).size(1000).all()
    ]);
...
async function onLoad(): Promise<void> {
    ...
    // Инициализация экземляров классов
    userDataManager = new UserDataManager();
    issues_manager = new IssuesManager();
    systemDataManager = new SystemDataManager();
    serializer = new Serializer();

    // Получение текущего пользорвателя и карточки сотрудников
    const [currentUser, allStaff] = await Promise.all([
        System.users.getCurrentUser(),
        Context.fields.staff_app.app.search().where(f => f.__deletedAt.eq(null)).size(10000).all(),
        getAllData()
    ]);

    // Посик карточки сотрудника текущего пользователя
    ...
    if (document.issuesWidgetInterval) {
        console.log("IssuesWidget: interval clear");
        window.clearInterval(document.issuesWidgetInterval);
    }
    // Создаем таймер
    document.issuesWidgetInterval = window.setInterval(() => {
        // Если ссылка отличается, то очищаем таймер
        if (window.location.href.indexOf(current_url) === -1) {
            console.log("IssuesWidget: refresh, interval clear");
            window.clearInterval(document.issuesWidgetInterval);
            return;
        }
        getOrRefreshData(true).then(_ => {
            console.log("IssuesWidget: refresh");
        });
    }, 60000);
}
