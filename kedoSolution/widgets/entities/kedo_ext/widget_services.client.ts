...
let all_service_manager: AllServiceManager;

class AllServiceManager {

    /** Отрисовка виджета */
    renderWidget() {

        // Получение шаблонов и контейнера всех заявок
        const issueListItemTemplate = document.querySelector(".new-issue_container_item-list-template");
        ...
        // Отрисовка групп заявок и самих заявок
        defaultIssues.forEach(issue => {
            ...
            if (issue.expandable && issue.issues) {
                ...
                issue.issues.forEach(listIssue => {
                    ...
                    // Добавление ссылки и кода
                    allServicesIssueItem.href = listIssue.link;
                    allServicesIssueItem.dataset["service_code"] = listIssue.code;

                    // Добавление иконки и названия заявки
                    [allServicesIssueItemIconContainer].forEach(item => this.changeElement(item, (<any>svgToTypeReference)[issue.issue_type!]));
                    [allServicesIssueItemLabel].forEach(item => this.changeElement(item, listIssue.name, true));
                    
                    // Отрисовка заявок в сервисе
                    this.renderComponent(newAllServicesColumnItem, allServicesIssueItem);
                });
                
                // Отрисовка контейнера сервиса в глваном контейнере
...
async function onLoad(): Promise<void> {
    all_service_manager = new AllServiceManager();
    all_service_manager.renderWidget();
}
